...
const schema = yup.object({
  ...
  role: yup.mixed<UserRole>().oneOf(["patient", "doctor", "receptionist", "admin"]).required("Role is required"),
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-accent via-background to-background p-4">
      <Card className="w-full max-w-md p-8">
        <div className="mb-6 flex flex-col items-center">
          ...
          <p className="text-sm text-muted-foreground">Pick a role to get started</p>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Input label="Password" type="password" placeholder="At least 6 characters" autoComplete="new-password" {...register("password")} error={errors.password?.message} />
          <Input label="Phone (optional)" placeholder="+1 555 0100" {...register("phone")} error={errors.phone?.message} />
          ...
          {role === "doctor" ? (
            <Input label="Specialty" placeholder="e.g. Pediatrics" {...register("specialty")} error={errors.specialty?.message} />
          ) : null}
